# ISA 2: measures of dispersion and visualizing relationships {#sec-ISACh2}
...
#| fig-cap: "Adding fit lines per region"
#| warning: FALSE
#| message: FALSE
wd %>%
  dplyr::mutate(Region = haven::as_factor(region)) %>%
...
### Barcharts for proportions

We can also use stacked barcharts to visualize percentage or proportion differences, to complement crosstabs.

```{r}
#| label: fig-propbar
#| fig-cap: "Stacked percentage barchart of region vs. press freedom (ordered by lowest-to-highest percentage 'Free')"
...
as.data.frame(prop.table(table(haven::as_factor(wd$fhp_status5),haven::as_factor(wd$region)),margin = 2)*100) %>%
  ...
  dplyr::arrange(desc(Free),desc(`Partly Free`),desc(`Not Free`)) %>%
  dplyr::mutate(Region=factor(Var2, Var2)) %>%
  tidyr::pivot_longer(-c(Var2,Region),names_to = "Freedom", values_to = "Freq") %>%
  ggplot2::ggplot(ggplot2::aes(x = Region, y = Freq, fill = Freedom)) +
